...
- 现有 PM skills 是单点方法，缺少场景化串联和门禁。

## 插件边界

PM Superpowers 不重复实现 68 个 PM skills。它只做上层编排：
...
现有 68 个 PM skills 是方法库，PM Superpowers 是流程系统。
...
1. 保留：不复制、不改名、不替换。
2. 归类：按产品发现、战略、执行、市场研究、数据分析、GTM、增长、AI shipping、工具类分组。
3. 编排：每个场景工作流引用一组底层技能。
4. 治理：PM Superpowers 对底层技能产物做证据、门禁和交接检查。
5. 扩展：缺的是场景流、门禁、handoff，不是重复造一遍方法技能。
